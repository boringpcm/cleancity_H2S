...
let requestedRole = 'user'; // 'user' or 'admin'
...
window.onload = async () => {
    ...
    if (window.authFunctions) {
        window.authFunctions.initAuthObserver();
    }
...
async function enterApp(role) {
    ...
    // Check admin access
    if (role === 'admin' && !window.authFunctions.isAdmin()) {
        showToast('Admin access denied. Please contact administrator.');
        return;
    }
...
async function handleSignIn() {
    ...
    if (!window.authFunctions) {
        showToast('Authentication not initialized');
        return;
    }
    ...
    if (result.success) {
        closeAuthModal();
        // Enter app with requested role
        enterApp(requestedRole);
    }
}
...
async function handleSignUp() {
    ...
    if (result.success) {
        closeAuthModal();
        // Enter app as regular user (admins must be promoted separately)
        enterApp('user');
    }
}
...
window.addEventListener('load', () => {
    ...
    if (logoutBtn) {
        logoutBtn.onclick = async () => {
            if (window.authFunctions) {
                await window.authFunctions.signOutUser();
            }
            location.reload();
        };
    }
